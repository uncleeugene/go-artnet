...
func (n *Node) pollReplyLoop() {
	...
	if err != nil {
		fmt.Printf("error creating ArtPollReply packet for self")
		return
	}
...
func (n *Node) sendLoop() {
	...
	for {
		...
		case payload := <-n.sendCh:
			...
			if err != nil {
				fmt.Printf("error writing packet")
				continue
			}
...
func (n *Node) RegisterCallback(opcode code.OpCode, callback NodeCallbackFn) {
	if !n.isShutdown() {
		fmt.Printf("ignoring callback registration: node has already been started")
		return
	}
